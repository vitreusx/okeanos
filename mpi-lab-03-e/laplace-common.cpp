...
bool InputOptions::isVerbose() { return this->verbose; }

int InputOptions::getErrorCode() { return this->errorCode; }

int GridFragment::getFirstRowIdxOwnedByProcess(int numPointsPerDimension,
...
GridFragment::GridFragment(int numPointsPerDimension, int numProcesses,
                           int myRank, bool debug) {
  int color, i;
  ...
  this->firstRowIdxIncl = this->getFirstRowIdxOwnedByProcess(
      numPointsPerDimension, numProcesses, myRank);
  this->lastRowIdxExcl = this->getFirstRowIdxOwnedByProcess(
      numPointsPerDimension, numProcesses, myRank + 1);
  numTotalRows = this->lastRowIdxExcl - this->firstRowIdxIncl + 2;
  maxPointsPerColorPerRow = (numPointsPerDimension + 1) / 2;

  if (debug) {
    std::cerr << "DBG PROCESS " << myRank << ": GD=" << this->gridDimension
              << " FR=" << this->firstRowIdxIncl
              << " LR=" << this->lastRowIdxExcl << " TR=" << numTotalRows
              << " MPPCPR=" << maxPointsPerColorPerRow << std::endl;
  }
  ...
  for (color = 0; color < 2; ++color) {
    ...
    for (i = 0; i < numTotalRows; ++i) {
      this->data[color][i] =
          new double[maxPointsPerColorPerRow]; // (double *)
...
void GridFragment::printEntireGrid(int myRank, int numProcesses) {
  if (myRank == 0) {
    MPI_Status status;
    int rowIdx = 0;

    for (rowIdx = this->firstRowIdxIncl; rowIdx < this->lastRowIdxExcl;
         ++rowIdx) {
      std::cout << std::fixed << std::setprecision(5) << GP(this, rowIdx, 0);

      for (int colIdx = 1; colIdx < this->gridDimension; ++colIdx) {
        std::cout << " " << std::fixed << std::setprecision(5)
                  << GP(this, rowIdx, colIdx);
      }
    ...
    for (int procIdx = 1; procIdx < numProcesses; ++procIdx) {
      int lastIdx = this->getFirstRowIdxOwnedByProcess(
          this->gridDimension, numProcesses, procIdx + 1);

      for (; rowIdx < lastIdx; ++rowIdx) {
        MPI_Recv(this->extraRowForPrinting, this->gridDimension, MPI_DOUBLE,
                 procIdx, PRINT_MSG_TAG, MPI_COMM_WORLD, &status);

        std::cout << std::fixed << std::setprecision(5)
                  << this->extraRowForPrinting[0];

        for (int colIdx = 1; colIdx < this->gridDimension; ++colIdx) {
          std::cout << " " << std::fixed << std::setprecision(5)
                    << this->extraRowForPrinting[colIdx];
        }
    ...
    for (int rowIdx = this->firstRowIdxIncl; rowIdx < this->lastRowIdxExcl;
         ++rowIdx) {
      for (int colIdx = 0; colIdx < this->gridDimension; ++colIdx) {
        this->extraRowForPrinting[colIdx] = GP(this, rowIdx, colIdx);
      }

      MPI_Send(this->extraRowForPrinting, this->gridDimension, MPI_DOUBLE, 0,
               PRINT_MSG_TAG, MPI_COMM_WORLD);
    }
  }
}
...
void GridFragment::initialize() {
  for (int rowIdx = this->firstRowIdxIncl; rowIdx < this->lastRowIdxExcl;
    ...
    for (int colIdx = 0; colIdx < this->gridDimension; ++colIdx) {
      GP(this, rowIdx, colIdx) =
          Utils::getInitialValue(rowIdx, colIdx, this->gridDimension);
    }
  }
}

int GridFragment::getNumColorPointsInRow(int rowIdx, int color) {
